...
class Inversions():
                ...
                for search_r in range(r, 3): # continue search from current row
                    if ( search_r == r ): # continue from current column in current row
                        for search_c in range(c, 3): # search columns to the right of current cell
                            ...
                            self.checkInversion(current_element, test_element)
    ...
    def checkInversion(self, current, test):
        print("\nCurrent:", current)
        print("Test:", test)
...
def isSolvable(puzzleInitial, puzzleGoal):
    # Count inversions in given 8 puzzle
    puzzle = Inversions()
    invCountInitial = puzzle.getInvCount(puzzleInitial)
    invCountGoal = puzzle.getInvCount(puzzleGoal)
...
puzzleInitial = [[2,8,3], [1,6,4], [7,0,5]] # Inversion Count: 11 -> odd
...
if (isSolvable(puzzleInitial, puzzleGoal)):
    print("Solvable")
else:
    print("Not Solvable")
